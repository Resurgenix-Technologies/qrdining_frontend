...
export default function Navbar() {
  ...
  const getStyle = (path, tabName) => {
    ...
    return `flex items-center gap-1.5 text-xs font-bold tracking-widest uppercase transition ${isActive ? 'text-black' : 'text-gray-400 hover:text-black'
      }`;
  };
  ...
  const getMobileStyle = (path, tabName) => {
    ...
    return `block text-xs font-bold tracking-widest uppercase py-2 ${isActive ? 'text-black' : 'text-gray-400'
      }`;
  };
